...
const PreReviewItem = ({ pullRequest }: { pullRequest: PullRequest }) => {
  ...
  return (
    <div
      ...
      onClick={() =>
        void router.push(`/pre-reviews/${pullRequest.id}`).catch(console.error)
      }
      key={pullRequest.id}
    >
      <div
      ...
      >
        <div className="p-4">
          ...
          <div className="text-sm text-slate-600">
            {pullRequest.owner}/{pullRequest.repo}/{pullRequest.head}
          </div>
          <div className="pt-3">
            <PreReviewStatusBadge status={pullRequest.status || "OPEN"} />
          </div>
        </div>
      </div>
    </div>
...
function PreReviewStatusBadge({ status }: { status: string }) {
  switch (status) {
    case "OPEN":
      return (
